...
// use the client NPCommandClient to send messages to execute
...
[assembly:AssemblyTitle("NPCommandService")]
[assembly:AssemblyDescription("Named Pipes command service")]
[assembly:AssemblyConfiguration("")]
[assembly:AssemblyCompany("MS")]
[assembly:AssemblyProduct("NPCommandService")]
...
namespace NaSpNPCommandService
{
	public partial class NPCommandService : ServiceBase
	{
		static int Main(string[] arguments)
		{ // entry point
			// declaration as int, transfer of parameters and return of a value are ignored when called via the service manager
			NPCommandService service = new NPCommandService();
			int rc = 0;
		...
		{ // called from service manager on service start
			...
			// check command line parameter of service, e.g. "sc start NPCommandService VERBOSE"
		...
		private void OnAsyncMessage(IAsyncResult result)
		{
			...
			if (bytesRead != 0)
			{
				...
				// execute the command received by named pipe
				string responseMessage = StartProcess(Encoding.UTF8.GetString(pd.data, 0, bytesRead), "");
		...
		#region action code
		private string StartProcess(string processName, string parameter)
		{ // function to start process

			// split process name and parameter since process and parameters are handed in processName and parameter is empty
			SplitFileName(ref processName, ref parameter);

			// configuration for the process to start
			...
			try {
				WriteVerboseToLog("Starting process " + processName);
				// start process
				process.Start();
				// important: read output and error before waiting for process exit to avoid deadlock situation
				string output = process.StandardOutput.ReadToEnd() + process.StandardError.ReadToEnd();
				// wait 'til process has stopped
				process.WaitForExit();
				// return output and errors of process
				return output;
			}
			catch (Exception ex)
			{ // error on starting process
		...
		// split path of executable and arguments
		public static void SplitFileName(ref string fileName, ref string arguments)
		{
			...
			// split process and parameters only if arguments are empty and filename exists
			if (string.IsNullOrEmpty(arguments) && !string.IsNullOrEmpty(fileName) && fileName.Length > 2)
			{
				// filename in double quotes?
				if (fileName.StartsWith(@""""))
				{ // string starts with double quotes, use double quote as separator between filename and parameters
				...
				{ // no double quotes, use space as separator between filename and parameters
