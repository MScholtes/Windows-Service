...
// use the client NPPowershellClient to send messages to execute
...
[assembly:AssemblyTitle("NPPowershellService")]
[assembly:AssemblyDescription("Named Pipes Powershell service")]
[assembly:AssemblyConfiguration("")]
[assembly:AssemblyCompany("MS")]
[assembly:AssemblyProduct("NPPowershellService")]
...
namespace NaSpNPPowershellService
{
	public partial class NPPowershellService : ServiceBase
	{
		[STAThread]
		static int Main(string[] arguments)
		{ // entry point
			// declaration as int, transfer of parameters and return of a value are ignored when called via the service manager
			NPPowershellService service = new NPPowershellService();
			int rc = 0;
		...
		int lineWidth = 0; // line width of powershell output (0 = no line width set)

		#region service structure

		public NPPowershellService()
		...
		{ // called from service manager on service start
			...
			// check command line parameter of service, e.g. "sc start NPPowershellService VERBOSE"
		...
		{ // read base configuration out of xml file: service and logging settings
			...
			try {
				...
				// width: line length, default: 0 (means no line width)
				tempStr = ReadXmlText(xmlDocument, "/serviceconfig/service/width", "0");
				if (Int32.TryParse(tempStr, out tempInt))
				{
					if (tempInt >= 0)
						lineWidth = tempInt;
					else
						WriteToLog("Invalid value for line width.");
				}
				else
					WriteToLog("Error when setting the line width.");
			}
			catch (Exception ex)
			{
		...
		private void OnAsyncMessage(IAsyncResult result)
		{
			...
			if (bytesRead != 0)
			{
				...
				// execute the powershell commands received by named pipe
				ExecutePowershell(Encoding.UTF8.GetString(pd.data, 0, bytesRead), out responseMessage, out errorMessage, lineWidth);
				// if command generated no output modify response
				responseMessage += errorMessage;
				if (responseMessage.Length == 0) responseMessage = "'" + Encoding.UTF8.GetString(pd.data, 0, bytesRead) + "' generated no output.";
				// store output in response data, make sure that no buffer overflow occurs
				pd.data = Encoding.UTF8.GetBytes(responseMessage.Substring(0, System.Math.Min(responseMessage.Length, SERVER_OUT_BUFFER_SIZE)));
				...
				{
					WriteToLog("Server instance " + (Int32) pd.state + " error, closing connection.");
					pd.pipe.Close();
				}
			}
		...
		#region action code
		// exeucte powershell commands in powershellConmmands
		// standard output is returned in standardOutput
		// error output is returned in errorOutput
		// returns true on success
		private bool ExecutePowershell(string powershellConmmands, out string standardOutput, out string errorOutput, int lineWidth)
		{
			errorOutput = "";

			// Standard initial session state erstellen (enthält alle Windows Powershell-Elemente.
			InitialSessionState initialsessionstate = InitialSessionState.CreateDefault();

			// ignoring Execution Policy in replacing the given  AuthorizationManager with an empty one
			initialsessionstate.AuthorizationManager = new System.Management.Automation.AuthorizationManager("MyShellId");

			// create Powershell runspace
			using (Runspace runspace = RunspaceFactory.CreateRunspace(initialsessionstate))
			{
				// use Single Threading Apartment (avoiding issued with COM)
				runspace.ApartmentState = System.Threading.ApartmentState.STA;
				// reuse current Thread verwenden -> works only if Main() is introduced with [STAThread]
				runspace.ThreadOptions = PSThreadOptions.UseCurrentThread;

				// open runspace
				runspace.Open();

				// send load errors to error channel
				foreach (var err in (ArrayList)runspace.SessionStateProxy.PSVariable.GetValue("Error"))
					errorOutput = err.ToString();

				// collection for command output
				var Ergebnisse = new Collection<PSObject>();

				// create a pipeline...
				using (Pipeline powershell = runspace.CreatePipeline())
				{
					// ...and add commands to it
					powershell.Commands.AddScript(powershellConmmands);

					// additional commandlet Out-String to format output
					Command command = new Command("Out-String");
					// line width
					if (lineWidth > 0)
						command.Parameters.Add("-Width", lineWidth.ToString());
					// add command to pipeline
					powershell.Commands.Add(command);

					try
					{ // execute commands
						Ergebnisse = powershell.Invoke();

						// collect runtime errors in string
						if (null != powershell.Error  && powershell.Error.Count > 0)
						{
							var pipelineError = new StringBuilder();
							foreach (var item in powershell.Error.ReadToEnd())
								pipelineError.AppendFormat("Error: {0}\r\n", item);
							errorOutput += pipelineError.ToString();
						}
					}
					catch (Exception ex)
					{ // error on call
						errorOutput = String.Format("Error: {0}\r\n", ex.Message);
					}
				}
				...
				var pipelineOutput = new StringBuilder();
				foreach (PSObject obj in Ergebnisse)
					pipelineOutput.Append(obj.ToString());
				standardOutput = pipelineOutput.ToString();

				// if no error occured return tue
